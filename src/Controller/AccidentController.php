...
*/
     ...
     * Method({"GET", "POST"})
     */
    public function newDetailaccident(Request $request,EntityManagerInterface $entityManager) {
    ...
    public function new(Request $request, EntityManagerInterface $entityManager,MailerInterface $mailer): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $email = (new Email())
                ...
                ->to($form->get('Mail_Assureur')->getData())
                ->subject('Attention nouveau accident')
                ->html('<p>see twig</p>');
            $mailer->send($email);
            $entityManager->flush();
    ...
    /**
     * @Route("/rechAccident",name="rechAccident")
     */
    public function searchaccident(Request $request,NormalizerInterface $Normalizer ){
        $repository=$this->getDoctrine()->getRepository(Accident::class);
        $requestString=$request->get('searchValue');
        $accidents=$repository->findAccidentByid($requestString);
        $jsonContent=$Normalizer->normalize($accidents,'json',['groups'=>'accidents']);
